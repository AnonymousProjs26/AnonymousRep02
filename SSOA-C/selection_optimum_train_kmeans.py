...
if __name__ == "__main__":
    ...
    dense_output_train = np.max(my_model.predict(X_train), axis=1)
    dense_output_train = dense_output_train.reshape(-1, 1)

    res_train = {}

    for i in range(len(dense_output_train)):
        temp = []
    ...
    for k in range(50):
        ...
        np.savetxt(os.path.join(basedir, "kmeans_con_{}/select{}.csv".format(exp_id, k)), np.array(acc_list1))
        np.savetxt(os.path.join(basedir, "kmeans_con_{}/random{}.csv".format(exp_id, k)), np.array(acc_list2))
